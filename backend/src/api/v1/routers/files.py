...
async def upload_file(
    ...
    ttl_hours: Optional[int] = Query(
        None, ge=1, le=720, description="文件生存时间(小时),最长30天,不设置则永久保存"
    ),
) -> FileUploadResponse:
    """
    上传文件到存储服务。
...
@router.get(
    "/{object_key}", summary="下载文件", description="通过对象键下载文件", response_class=Response
)
async def download_file(object_key: str = Path(..., description="文件对象键")) -> Response:
    """
    下载文件。
...
@router.delete("/{object_key}", summary="删除文件", description="删除指定的文件")
async def delete_file(object_key: str = Path(..., description="文件对象键")) -> dict[str, str]:
    """
    删除文件。
