...
from src.domain.enums.print_enums import AdapterType, PrinterStatus
...
class Printer(BaseModel):
    ...
    def is_available(self) -> bool:
        ...
        """
        return (
            self.is_enabled and self.status == PrinterStatus.IDLE and self.current_task_id is None
        )
    ...
    def assign_task(self, task_id: UUID) -> None:
        ...
        if not self.is_available():
            raise PrinterBusyError(f"Printer {self.id} is not available (status: {self.status})")
